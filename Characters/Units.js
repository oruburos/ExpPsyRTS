...
//MOVETO
	...
	Unit.allOurUnits=function(){
		return Unit.allUnits.filter(function(chara){
			...
			return !(chara.isEnemy())  && (!chara.isMineral);
		});
	};
	...
	var AttackableUnit=Unit.extends({
		...
		prototypePlus:{
							...
							if (myself.isReloaded && myself.isReloaded() && myself.isInAttackRange(enemy) && myself.status=="dock") {
								...
								//Else will cause damage immediately (melee attack)
									...
									if (myself.AOE) {
										...
										//	console.log("damage " + chara.name  + " by " + myself.name )
			...
			findNearbyTargets:function(){
				...
				var targets=[
					units.filter(function(chara){
					   // return chara.attack }),
						return !chara.isMineral && chara.attack  }),
						//[],//cambio para que no ataque command center
			  ...
			  findNearbyMinerals:function(){
				...
				var targets=[
					units.filter(function(chara){
					  //  return chara.attack }),
					   //console.log("chara"+chara.name + " c " + chara.isMineral )
						return chara.isMineral  })
				];
				...
				targets.forEach(function(charas){
					...
					//console.log( "pos mia " + myX + " , " + myY + " posicion mineral " + chara.posX() + " , "+ chara.posY() + " puedo verlo " +myself.canSee(chara));
						return myself.canSee(chara) /*aqui ira que no este cargando && myself.matchAttackLimit(chara)*/;
					}).sort(function(chara1,chara2){
						var X1=chara1.posX(),Y1=chara1.posY(),X2=chara2.posX(),Y2=chara2.posY();
						return (X1-myX)*(X1-myX)+(Y1-myY)*(Y1-myY)-(X2-myX)*(X2-myX)-(Y2-myY)*(Y2-myY);
					});
			...
			highestPriorityTarget:function(){
			 ...
			 highestPriorityMineral:function(){
				return this.findNearbyMinerals()[0];
			},
			AI:function(){
			...
			collect:function(mineral){
			 ...
			 return;}
				if (mineral instanceof Gobj && mineral.status!="dead") {
					...
					if (this.meleeAttack) {
						//Override
						this.attackRange=this.radius()+mineral.radius();
						//   console.log("aas")
					}
					//If separate attack mode, override
					if (this.attackMode) {
						console.log("atkac")
						this.Bullet=(mineral.isFlying)?this.attackMode.flying.Bullet:this.attackMode.ground.Bullet;
					...
					if (this.tracing) {
						//Adjust attack range for tracing
						range=Math.max(this.get('attackRange')*0.7,(this.radius()+mineral.radius())*0.6);
						delete this.tracing;
					}
					//Add to fix holding issue
					if (!this.hold) this.moveToward(mineral,range);
					var myself=this;
					var attackFrame=function(){
						//If mineral already dead or becomes invisible or we just miss mineral
						if (mineral.status=="dead" || mineral['isInvisible'+myself.team] || myself.isMissingTarget()) {
							myself.stopAttack();
							myself.dock();
							//console.log("dock 2")
						}
								...
								if (myself.AOE) {
									...
									switch(myself.attackLimit){
										...
										case "ground":
											...
											var mineralBuildings=Building.allBuildings.filter(function(chara){
												return chara.team!=myself.team;
											});
											enemies=mineralUnits.concat(mineralBuildings);
											break;
										default:
											enemies=(Unit.allUnits.concat(Building.allBuildings)).filter(function(chara){
												return chara.team!=myself.team;
